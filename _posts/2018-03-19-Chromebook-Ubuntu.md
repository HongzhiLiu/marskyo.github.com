...
- 打开Github(https://github.com/dnschneid/crouton)，下方说明有详细步骤，懂英文的直接看页面教程，这步后续就不用看了。

- 查看页面从这个网址https://goo.gl/fd3zc下载Crouton到Chromebook的downloads下载目录。
...
- 然后通过下面命令行来开始运行Ubuntu：`sudo startxfce4`（**提示：**如果Chrome浏览器重Crosh终端重启后，还是要先输入 `shell`，再输入上述运行命令进入Ubuntu系统）。
